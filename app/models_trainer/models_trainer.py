...
from fastai.vision.all import Path, L, download_images
...
async def train_model(trainer_id):
    ...
    download_images_f(trainer_id, trainer_images)


def download_images_f(trainer_id: str, trainer_images: list[Trainer_images_by_category_response]):
    path = Path(f'app/models_trainer/{trainer_id}')
    ...
    for image in trainer_images:
        ...
        download_images(dest, urls=L(urls))